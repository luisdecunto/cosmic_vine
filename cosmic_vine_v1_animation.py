...
n_gen = 50                                 # How many cubes or "generations" does the sequence run
fname = f"cosmic_vine_{n_gen}_cubes.obj"    # Name of the output object
fps = 2                                    # Frames per second for the animation
animation = True                           # Set to 'ffmpeg' for mp4, 'html' for html, or False to skip
...
if animation:
    ...
    # Compute axis limits from all cube centers
    all_centers = np.array([c.local_csys[:3, 3] for c in vine])
    margin = 2.0
    lo = all_centers.min(axis=0) - margin
    hi = all_centers.max(axis=0) + margin

    def update(frame):
        ax.cla()
        # Draw cubes up to current frame
        for i in range(frame + 1):
            for p0, p1 in vine[i].edges():
                ax.plot3D(*zip(p0, p1), color='steelblue', linewidth=0.5)
        # Draw skeleton path
        for i in range(frame):
            c0 = vine[i].local_csys[:3, 3]
            c1 = vine[i + 1].local_csys[:3, 3]
            ax.plot3D(*zip(c0, c1), color='red', linewidth=1)
        ax.set_xlim(lo[0], hi[0])
        ax.set_ylim(lo[1], hi[1])
        ax.set_zlim(lo[2], hi[2])
        ax.set_box_aspect([1, 1, 1])
        ax.set_title(f'Cube {frame + 1} / {len(vine)}')
        print(f"\rFrame {frame + 1}/{len(vine)}", end="")

    anim = FuncAnimation(fig, update, frames=len(vine), interval=1000//fps, repeat=False)
    if animation == True:
        anim_file = f"cosmic_vine_{n_gen}_cubes.mp4"
        anim.save(anim_file, writer='ffmpeg', fps=fps)
    elif animation=='html':
        anim_file = f"cosmic_vine_{n_gen}_cubes.html"
        anim.save(anim_file, writer='html')
    plt.close(fig)
    print(f"\nAnimation saved to {anim_file}")
